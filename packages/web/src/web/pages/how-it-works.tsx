...
const FAQ = [
  ...
  { q: "Why does Redeem read at signing rather than at the record date?", a: "The public Robinhood Chain RPC keeps no historical state, so counted weight is read live at signing and re-checked at cutoff, and every receipt says which block. Redeem also rebuilds what a wallet held on the issuer's record date from the chain's own Transfer logs (current balance minus net transfers since) and shows it next to your intent. When the log scan cannot reach that far back the figure is withheld, never guessed." },
  { q: "What about tokens sitting in liquidity pools?", a: "Redeem looks through them. A discovery scan finds contracts that hold Stock Tokens and classifies the ones it can read; today that is 29 Uniswap V3 style pools across 13 tokens. Each record page shows how many share-equivalents sit in pools rather than wallets. Concentrated-liquidity positions are resolved to the pool, not yet to each provider; V2 pairs and ERC-4626 vaults can be resolved to a wallet pro rata. All of it is view calls; nothing is deployed." },
  { q: "What does the REDEEM token unlock?", a: "Delivery only: signed webhooks that receive record events as they happen (1 webhook from 100,000 REDEEM, 5 from 1,000,000). The balance is read from the chain and nothing is locked or spent. It never changes intent weight, queue positions, record numbers or XP, and the record, the API, the reports and the verifier stay free for everyone." },
  ...
  { q: "What does attestation prove?", a: "That the tally published at cutoff is the tally of exactly these receipts, in this order, with these weights, unchanged since. Each receipt carries its Merkle proof. At cutoff the attestation and the whole meeting report are also frozen as canonical JSON, hashed, and timestamped into Bitcoin through OpenTimestamps, so anyone can prove they existed then and have not changed, with the standard ots client and without trusting Redeem. No contract is involved." },
  { q: "What is a Holder Intent Report?", a: "One document per stockholder meeting: what the holders of that company's Stock Token recorded on each proxy item, next to the board's recommendation. It is live until the cutoff and then frozen, hashed and timestamped. It describes recorded intent. It is not a shareholder vote and not a proxy solicitation, and it asks nobody to vote, grant a proxy or take any action at a meeting." },
  { q: "What is the founding hundred?", a: "The first hundred wallets to record an intent or pre-register for redemption. Wallet numbers come from signing order and never change, so the list fills once, in order. The genesis page is the permanent file of those wallets and of the first intent, the first queue request and the first attested tally. It is a place in a file, not a token, a vote or a claim." },
  ...
  { q: "What is a record date?", a: "The day an issuer fixes who may vote at a meeting. Whoever is a holder of record at the close of that day gets the ballot, even if they sell the next morning. It is stated in the proxy statement, and Redeem shows it only where the filing states it. The meeting calendar lists every one on file." },
  { q: "Who counts my intent, and can Redeem change it?", a: "Anyone can count it. Every receipt carries the wallet's own signature over the exact choice and weight, so a changed choice would no longer verify. Redeem cannot sign for a wallet, and the verifier page checks all of this in your browser without asking Redeem anything." },
  { q: "What happens at the cutoff?", a: "Intent closes three days before the meeting. Every wallet's balance is read again and the smaller of the signed and the held share-equivalent counts. The receipts are hashed into a Merkle root, the tally and the meeting report are frozen and timestamped, and after the meeting the issuer's reported result is added beside them when its Form 8-K appears." },
  { q: "Why can a company show items I cannot sign?", a: "Signing needs a position. If the wallet holds none of that company's Stock Token there is no share-equivalent to weigh the intent with, so the items are readable and the sign button stays off. Closed items can never be signed." },
  { q: "Why does my ETF have no items?", a: "Funds vote the shares in their own portfolios and do not send a company proxy to their holders the way an operating company does, so there is nothing to record intent on. Foreign issuers are similar: they publish meeting materials on Form 6-K rather than a DEF 14A proxy statement, and are marked that way in Markets." },
  { q: "Where do the 'how shareholders voted' numbers come from?", a: "From the issuer's own Form 8-K, Item 5.07, which it must file within four business days of the meeting. Redeem extracts the counts and links the filing; where the two differ the filing governs. Those are votes of holders of record. Token-holder intent was not part of them and is never mixed into them." },
